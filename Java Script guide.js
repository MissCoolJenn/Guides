...
// Если вы запускаете код в браузере, поскольку модуль fs является частью Node.js и не доступен в среде браузера.

// Загрузка можуля файловой системы из Node.js 
const fs = require('fs');

// Создание файла
// сихнронно (блокирует программу пока идет работа c файлом)
fs.writeFileSync('test.txt', 'Привет, мир!');
// асинхронно
fs.writeFile('test.txt', 'Привет, мир!');
...
// Перезапись файла
fs.writeFileSync('test.txt', 'Новое содержимое файла');
